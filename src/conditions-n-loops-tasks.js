...
function convertToRomanNumerals(num) {
  ...
  let result = '';
  let newNum = num;
  for (let i = 0; i < arabicNumerals.length; i += 1) {
...
function getSpiralMatrix(size) {
  ...
  while (firstRow <= lastRow && firstCol <= lastCol) {
    for (let j = firstCol; j <= lastCol; j += 1) {
    ...
    lastCol -= 1;
    for (let j = lastCol; j >= firstCol; j -= 1) {
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < n; i += 1) {
    for (let j = 0; j < n; j += 1) {
      rotatedMatrix[i][j] = newMatrix[n - j - 1][i];
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 1; i <= iterations; i += 1) {
    ...
    newSrt = oneStr + twoStr;
    if (newSrt === str) {
      const newIteration = iterations % i;
      return shuffleChar(str, newIteration);
    }
  }
  return newSrt;
}
